class MessagesController < ApplicationController
  ...
  include Rails.application.routes.url_helpers # Needed for URL helpers
  ...
  def create
    ...
    begin
      ...
      if message_params[:attachment]
        ...
        att_info = { 
          filename: (att.respond_to?(:original_filename) ? att.original_filename : nil), 
          content_type: (att.respond_to?(:content_type) ? att.content_type : nil), 
          size: (att.respond_to?(:size) ? att.size : nil) 
        }
  ...
  def build_webhook_payload(message)
    attachments = if message.attachment.attached?
      message.attachment.map do |att|
        {
          filename: att.filename.to_s,
          content_type: att.content_type,
          byte_size: att.byte_size,
          url: rails_blob_url(att, only_path: false)
        }
      end
    else
      []
